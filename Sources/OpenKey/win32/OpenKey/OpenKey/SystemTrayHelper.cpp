...
#define POPUP_ABOUT_vie-type 1010
#define POPUP_vie-type_EXIT 2000
...
map<UINT, LPCTSTR> menuData = {
	...
	{POPUP_ABOUT_vie-type, _T("Giới thiệu vie-type")},
	{POPUP_vie-type_EXIT, _T("Thoát")},
};
	...
	case WM_TRAYMESSAGE: {
		...
		if (lParam == WM_LBUTTONUP) {
			...
			case POPUP_CONTROL_PANEL:
				AppDelegate::getInstance()->onControlPanel();
				break;
			case POPUP_ABOUT_vie-type:
				AppDelegate::getInstance()->onvie-typeAbout();
				break;
			case POPUP_vie-type_EXIT:
...
void SystemTrayHelper::createPopupMenu() {
	...
	AppendMenu(popupMenu, MF_UNCHECKED, POPUP_ABOUT_vie-type, menuData[POPUP_ABOUT_vie-type]);
	AppendMenu(popupMenu, MF_SEPARATOR, 0, 0);
	AppendMenu(popupMenu, MF_UNCHECKED, POPUP_vie-type_EXIT, menuData[POPUP_vie-type_EXIT]);
